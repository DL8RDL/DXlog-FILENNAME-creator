# ==========================================================================
# EINGEBETTETE STANDARD-CONFI-VORLAGE (Ganz oben im Skript platziert)
# ==========================================================================
...
This program generates directory- and filenames (*.dxn) in your file-system
...
Therefore this small program was written by me.
...
Below this path the program generates one more directory named with a year-figure, e.g. 2026
...
It won´t do it for you. But DXlog FILENAME-creator does!
...
You select the empty file and save your contest in that file.
...
Кубок России на КВ телефон
Открытый Чемпионат Удмуртии
Чемпионат РФ CW
Чемпионат РФ SSB
'@
}
...
if (-not (Test-Path $configFile)) {
    ...
    # Das rote Warn-Label mit Ihrer ersten Frage
    ...
    $errorForm.Controls.Add($btnErrYes)
        ...
        # Der NEIN-Button der ersten Box -> Öffnet das identische zweite Fenster
    ...
    $btnErrNo.Add_Click({
        ...
        # Das zweite Fenster im EXAKT GLEICHEN LAYOUT
        ...
        # Rotes Warn-Label für die zweite Box
        ...
        # Der englische YES-Button für die zweite Box (Grün)
        ...
        # Der englische NO-Button für die zweite Box
...
# --- Das lokale BCC Logo rechts oben ---
...
function Update-PathYear {
    ...
    # Wenn wir die saubere Basis noch nicht haben, holen wir sie uns jetzt
    if ([string]::IsNullOrWhiteSpace($script:userBasePath)) {
        $script:userBasePath = $txtPath.Text
    }

    # Wir säubern die gemerkte Basis von eventuellen Jahreszahlen am Ende
    ...
    # SPERRE AKTIVIEREN: Pfad-Event stummschalten
    ...
    # SPERRE DEAKTIVIEREN: Pfad-Event wieder freigeben
    $script:ignorePathChange = $false
}


# --- MENÜ-LOGIK (Help & About) ---
...
$txtPath.Add_TextChanged({ 
    # Wenn die Sperre aktiv ist (weil das Jahr getippt wurde), ignorieren wir das!
    if ($script:ignorePathChange) { return }
    
    # Nur wenn der User SELBST tippt, merken wir uns den neuen Pfad als Basis
    $script:userBasePath = $txtPath.Text 
})

$txtPath.Text = $savedPath; $txtPath.BackColor = [System.Drawing.Color]::FromArgb(220, 255, 255); $txtPath.Location = New-Object System.Drawing.Point(20,130); $txtPath.Size = New-Object System.Drawing.Size(800,20); $form.Controls.Add($txtPath)

# 2. Rufzeichen + grauer, kursiver Hinweis
...
# 7. Button zum Erstellen + Visuelles 3D-Fokus-Grün + Mouseover
...
# 8. DXLog Pfad-Konfiguration & Footer
...
$txtDxLogPath.Text = $savedDxLogPath; $txtDxLogPath.BackColor = [System.Drawing.Color]::FromArgb(220, 255, 255); $txtDxLogPath.Location = New-Object System.Drawing.Point(20,640); $txtDxLogPath.Size = New-Object System.Drawing.Size(800,20); $form.Controls.Add($txtDxLogPath)
...
$btnCreate.Add_Click({
    $targetPath = $txtPath.Text.Trim(); $currentCall = $txtCall.Text.Trim(); $currentYear = $txtYear.Text.Trim() -replace '[^0-9]'; $name = $txtName.Text.Trim(); $dxLogExePath = $txtDxLogPath.Text.Trim()
    if ([string]::IsNullOrWhiteSpace($name) -or $name -like "*CONTESTNAME*" -or [string]::IsNullOrWhiteSpace($currentCall) -or [string]::IsNullOrWhiteSpace($currentYear)) { 
        [System.Windows.Forms.MessageBox]::Show("Bitte fülle alle Felder korrekt aus!", "Fehler", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning); return 
    }
    ...
    $fullFolder = Join-Path $targetPath $name; $fileNameWithExt = "$name.dxn"; $fullFile = Join-Path $fullFolder $fileNameWithExt
    try {
        if (-not (Test-Path $fullFolder)) { $null = New-Item -ItemType Directory -Path $fullFolder -Force }
        $null = New-Item -ItemType File -Path $fullFile -Force

        
        Set-Clipboard -Value $fullFolder
        $lblLivePathPreview.Visible = $true; $lblLivePathPreview.Refresh()
        $lblCopiedFeedback.Visible = $true; $lblCopiedFeedback.Refresh()
        ...
        if ($msgBoxResult -eq "Yes") { if (Test-Path $dxLogExePath) { Start-Process $dxLogExePath } else { [System.Windows.Forms.MessageBox]::Show("DXLog wurde unter dem angegebenen Pfad nicht gefunden!", "Fehler", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error) } }
        $form.Close() 
    } catch { [System.Windows.Forms.MessageBox]::Show("Fehler beim Erstellen:`n$($_.Exception.Message)", "Fehler", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error) }
})
...
# =====================================================================
# NACHTRÄGLICHE ERWEITERUNG: BROWSE-BUTTONS FÜR DIE TEXTFELDER
# =====================================================================

# 1. Hauptfenster verbreitern, damit die Buttons Platz haben
$form.Width = 460

# 2. Oberen Browse-Button erstellen
...
# Setzt den Button automatisch genau rechts neben Ihr Textfeld:
...
# 3. Unteren Browse-Button erstellen
...
# Setzt den Button automatisch genau rechts neben Ihr finales Textfeld:
