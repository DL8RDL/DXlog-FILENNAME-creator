...
function Get-DefaultIniTemplate {
    return @'
# ===========================================================================================
# DXlog FILENAME-creator 1.0 by DL8RDL (c) 2026 - configuration file
...
Just copy the following files together in the same directory: 
...
# when you save this ini-file, make sure that it is encoded in UTF-8
# if it´s not UTF-8 you´ll be in trouble concerning several wrong characters
#
# the following list contains supported contests by DXlog.
# current list: updated in August 2026
#
# keep in mind: if you edit this file and you lose it all your freshly
# entered data will be lost, in case the program creates a new, default ini.
#
<choose your contest>
10-10 QSO Party
...
$menuAbout.Add_Click({
    $aboutText = "DXlog FILENAME-creator 1.0 by DL8RDL,`r`ninspired by Martin, DL5RMH. Check out DL5RMH´s DXlog-tools an further more here: https://github.com/dl5rmh`r`n`r`nCreated between QSOs with incredible help from Google.`r`n`r`n© 2026 DL8RDL`r`n`r`nThis small program may be distributed by anyone, anywhere for free. The author assumes no liability for any damage caused by this program. Powered by Ham-spirit.`r`n`r`nDXlog is a relieable, state-of-the-art contest-logging software based on the very well known contest-logger 'WinTest'.`r`n`r`nFind DXlog here: https://www.dxlog.net/"
...
$btnCreate.Add_Click({
    ...
    try {
        ...
        $msgBoxResult = [System.Windows.Forms.MessageBox]::Show("Successfully created!`n`nOrdner: $fullFolder`n`nThe path has been copied to clipboard!`n`nDo you want to launch DXlog now?", "Erfolg", [System.Windows.Forms.MessageBoxButtons]::YesNo, [System.Windows.Forms.MessageBoxIcon]::Question, [System.Windows.Forms.MessageBoxDefaultButton]::Button1)
        
        if ($msgBoxResult -eq "Yes") { 
            if (Test-Path $dxLogExePath) { 
                Start-Process $dxLogExePath 
            } else { 
                [System.Windows.Forms.MessageBox]::Show("DXlog not found at the given path!", "Fehler", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error) 
            } 
        }
